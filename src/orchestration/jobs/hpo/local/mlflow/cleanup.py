...
def should_skip_cleanup(hpo_config: Dict[str, Any]) -> tuple[bool, str]:
    ...
    skip_cleanup_config = cleanup_config.get("disable_auto_cleanup", True)  # Default: true (disabled)
    skip_cleanup_env = os.environ.get("DISABLE_AUTO_CLEANUP", "").lower() == "true"
...
def cleanup_interrupted_runs(
    ...
    try:
        ...
        if not experiment:
            logger.warning(f"[CLEANUP] Could not retrieve experiment: {mlflow_experiment_name}")
            return None
        ...
        try:
            ...
            logger.warning(f"[CLEANUP] Error fetching runs: {e}. Cleanup may be incomplete.")
            all_runs = []
            ...
            for run in interrupted_parents:
                ...
                try:
                    ...
                    for child_run in child_runs:
                        ...
                        try:
                            client.set_tag(child_run_id, "code.interrupted", "true")
                            tagged_children += 1
        ...
        logger.warning(f"[CLEANUP] Error during cleanup of interrupted runs: {e}")
        import traceback
