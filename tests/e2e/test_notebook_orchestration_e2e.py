...
def validate_path_structure(path: Path, pattern_type: str, config_dir: Path) -> bool:
    ...
    Args:
        ...
        pattern_type: Type of pattern (e.g., "hpo_v2", "benchmarking_v2")
    ...
    try:
        ...
        # e.g., '{storage_env}/{model}/study-{study8}' -> check for study- prefix
        path_str = str(path)
        if "study-" in pattern:
...
def validate_run_name(run_name: str, process_type: str, config_dir: Path) -> bool:
    ...
    Args:
        ...
        process_type: Process type (e.g., "hpo", "benchmarking", "training")
    ...
    try:
        ...
        # This is a simplified check - full validation would require parsing the template
...
def validate_tags(tags: dict, process_type: str, config_dir: Path) -> tuple:
    ...
    try:
        ...
        # Note: Tags may use "code.*" namespace instead of process.*
        if process_type == "hpo":
            # HPO should have process tags (may be code.stage, code.project, etc.)
            required_tags = [
                ("process", "stage"),  # May be code.stage
                ("process", "project"),  # May be code.project
                ("process", "model"),  # May be code.model
                # run_type may be mlflow.runType or azureml.runType, not code.run_type
            ]
        elif process_type == "benchmarking":
            # Benchmarking should have process tags
            # Note: backbone may be in code.model or code.backbone
            required_tags = [
                ("process", "stage"),  # May be code.stage
                ("process", "project"),  # May be code.project
                ("process", "model"),  # May be code.model (backbone info)
            ]
        ...
        # If validation fails, return True with warning (don't fail tests)
